...
class REFER:
	...
	def createIndex(self):
		...
		for ann in self.data['annotations']:
			# ann is a dict  
			Anns[ann['id']] = ann
			imgToAnns[ann['image_id']] = imgToAnns.get(ann['image_id'], []) + [ann] # an image may have many annotations  #
		for img in self.data['images']:
			Imgs[img['id']] = img
		for cat in self.data['categories']:
			Cats[cat['id']] = cat['name'] # collec 收集类名
	...
	def getAnnIds(self, image_ids=[], cat_ids=[], ref_ids=[]):
		...
		if len(image_ids) == len(cat_ids) == len(ref_ids) == 0:
			...
			if not len(image_ids) == 0:
				...
				anns = list(itertools.chain.from_iterable(lists)) # [[anns],[anns]] => [anns, anns]
			else:
			...
			if not len(ref_ids) == 0:
				ids = set(ann_ids).intersection(set([self.Refs[ref_id]['ann_id'] for ref_id in ref_ids])) #交集 ids-> ann_ids
				ann_ids = list(ids)
		return ann_ids
	...
	def getImgIds(self, ref_ids=[]):
		...
		if not len(ref_ids) == 0:
			image_ids = list(set([self.Refs[ref_id]['image_id'] for ref_id in ref_ids]))# 去重
		else:
			image_ids = self.Imgs.keys() #所有 图像 的ids
		return image_ids

	def getCatIds(self):
		return self.Cats.keys() #所有 类别 的ids

	def loadRefs(self, ref_ids=[]):
		if type(ref_ids) == list: # multiple
			return [self.Refs[ref_id] for ref_id in ref_ids] 
		elif type(ref_ids) == int:  #single
			return [self.Refs[ref_ids]]

	def loadAnns(self, ann_ids=[]):
		if type(ann_ids) == list:
			return [self.Anns[ann_id] for ann_id in ann_ids]
		elif type(ann_ids) == int or type(ann_ids) == str:   # replace unicodde -> str
